...
from tqdm import tqdm  # 新增导入 tqdm
...
def _rr_selection_single(trials, t_stimulus=cfg.exp_info["t_stimulus"], l=cfg.exp_info["l_stimulus"], reliability_threshold=0.6, snr_threshold=0.8, effect_size_threshold=0.5, response_ratio_threshold=0.6, class_label="All"):
    ...
    inhibitory_responsive_mask = ((response_ratio > response_ratio_threshold)&(np.mean(stimulus_mean > baseline_mean, axis=0) > response_ratio_threshold))
    inhibitory_responsive_idx = np.where(inhibitory_responsive_mask)[0].tolist()
    ...
    # **类别 2: 兴奋+抑制性 RR 细胞 (Reliable AND inhibitory Responsive)**
    rr_inhibitory_mask = reliable_neurons_mask & inhibitory_responsive_mask
    rr_inhibitory_idx = np.where(rr_inhibitory_mask)[0].tolist()
    
    
    # 返回: 1/2. 最终RR索引, 3. 可靠性索引, 4/5. 响应性索引
    return rr_excitatory_idx, rr_inhibitory_idx
...
def rr_selection_by_class(segments, labels, **kwargs):
    ...
    all_rr_excitatory = set()
    all_rr_inhibitory = set()
    ...
    for class_label in unique_classes:
        ...
        # 获取当前类别的试次
        class_trials = segments[labels == class_label]
        
        # 调用单类别筛选
        rr_excitatory, rr_inhibitory = _rr_selection_single(class_trials, **kwargs)
        
        # 合并结果
        all_rr_excitatory.update(rr_excitatory)
        all_rr_inhibitory.update(rr_inhibitory)
    
    return sorted(list(all_rr_excitatory)), sorted(list(all_rr_inhibitory))
...
if __name__ == "__main__":
    ...
    # 尝试从缓存文件加载数据
    segments, labels, neuron_pos = None, None, None
    if os.path.exists(cache_file):
        segments, labels, neuron_pos = load_preprocessed_data_npz(cache_file)

    # 如果缓存文件不可用，则执行完整的加载和预处理流程
    if segments is None or labels is None or neuron_pos is None:
        ...
        print("成功从缓存文件加载数据，跳过数据加载和预处理步骤。")
    ...
    # 步骤 4: 进入循环，调用 rr_selection_by_class 替代原 rr_selection
    for rel, snr, eff, res in tqdm(all_combinations, desc="参数组合测试进度", unit="组合"):
        rr_enhanced_neurons, rr_inhibitory_neurons = rr_selection_by_class(
            segments, 
            np.array(labels), 
        ...
        # 提取数量
        N_RR_ENHANCED = len(rr_enhanced_neurons)
        N_RR_inhibitory = len(rr_inhibitory_neurons)

        # 存储兴奋性结果
        results_excitatory.append([rel, snr, eff, res, N_RR_ENHANCED])
        
        # 存储抑制性结果
        results_inhibitory.append([rel, snr, eff, res, N_RR_inhibitory])
    ...
    columns = ['rel_thresh', 'snr_thresh', 'eff_thresh', 'res_thresh', 'N_RR_Final']
    ...
    # 5.2 保存 inhibitory 结果
    df_inhibitory = pd.DataFrame(results_inhibitory, columns=columns)
    inhibitory_path = os.path.join(cfg.data_path, "RR_Test_Inhibitory.csv")
    df_inhibitory.to_csv(inhibitory_path, index=False)
    print(f"结果已保存至: {inhibitory_path}")