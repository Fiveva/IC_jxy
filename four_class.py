...
def rr_selection_by_class(segments, labels, **kwargs):
    ...
    global_rr_inhibitory_set = set() # <-- **改为抑制性集合**
    ...
    for class_id in valid_class_ids:
        ...
        # _rr_selection_single 返回 rr_enhanced_neurons 和 rr_inhibitory_neurons
        rr_exc_local_indices, rr_inh_local_indices = _rr_selection_single( # <-- **改为抑制性索引**
            class_segments, 
        ...
        global_rr_inhibitory_set.update(rr_inh_local_indices) # <-- **更新抑制性集合**

    # 结果转为列表并排序
    rr_enhanced_neurons = sorted(list(global_rr_enhanced_set))
    rr_inhibitory_neurons = sorted(list(global_rr_inhibitory_set)) # <-- **更新抑制性列表**
    
    elapsed_time = time.time() - start_time
    print(f"\n分类别 RR 筛选完成，总耗时: {elapsed_time:.2f}秒")
    print(f"最终筛选结果 (取并集): 兴奋性RR神经元总数: {len(rr_enhanced_neurons)}, 抑制性RR神经元总数: {len(rr_inhibitory_neurons)}") # <-- **更新打印信息**
...
def save_rr_neuron_indices_to_csv(rr_enhanced_neurons, rr_inhibitory_neurons, save_path): # <-- **函数签名更新**
    """
    将兴奋性RR神经元和抑制性RR神经元的原始索引保存到CSV文件中。
    
    参数:
    rr_enhanced_neurons: 兴奋性RR神经元（增强响应）的原始索引列表/数组。
    rr_inhibitory_neurons: 抑制性RR神经元（抑制响应）的原始索引列表/数组。 # <-- **参数描述更新**
    save_path: CSV文件的完整保存路径。
    """
    
    # 转换为列表
    exc_list = list(rr_enhanced_neurons)
    inh_list = list(rr_inhibitory_neurons) # <-- **变量名更新**
    
    # 填充较短的列表使其长度相等，以便创建DataFrame
    max_len = max(len(exc_list), len(inh_list))
    
    # 使用 NaN 或 None 填充
    exc_list += [None] * (max_len - len(exc_list))
    inh_list += [None] * (max_len - len(inh_list)) # <-- **变量名更新**
    
    # 创建 DataFrame
    df = pd.DataFrame({
        '兴奋性神经元序号': exc_list,
        '抑制神经元序号': inh_list # <-- **列名和变量更新**
    })
    
    # 保存为CSV文件，不包含索引
    df.to_csv(save_path, index=False)
    print(f"\n成功保存 RR 神经元序号索引文件: {save_path}")
    return True
...
if __name__ == "__main__":
    ...
    # =========================================================================
    # 新增功能: 生成 RR 神经元序号索引 CSV 文件
    # =========================================================================
    csv_filename = 'rr_neuron_indices.csv'
    csv_save_path = os.path.join(cfg.data_path, csv_filename)
    
    # 调用更新后的函数
    save_rr_neuron_indices_to_csv(rr_enhanced_neurons, rr_inhibitory_neurons, csv_save_path) 
    # =========================================================================


    # 提取兴奋性 RR 神经元的数据 (后续可视化/分类任务可能需要)
    enhanced_segments = segments[:, rr_enhanced_neurons, :]
    enhanced_neuron_pos_rr = neuron_pos_filtered[:, rr_enhanced_neurons]

    # 提取抑制性 RR 神经元的数据 (后续可视化/分类任务可能需要)
    inhibitory_segments = segments[:, rr_inhibitory_neurons, :] # <-- **变量名更新**
    inhibitory_neuron_pos_rr = neuron_pos_filtered[:, rr_inhibitory_neurons] # <-- **变量名更新**

    # %% 可视化RR神经元分布 (兴奋性)
    # 传入过滤后的全部神经元位置 (neuron_pos_filtered) 作为背景
    _rr_distribution_plot(neuron_pos_filtered, enhanced_neuron_pos_rr, plot_dir, "Excitatory_ByClass")
    # %% 可视化RR神经元响应 (兴奋性)
    _plot_rr_responses(enhanced_segments, labels, plot_dir, "Excitatory_ByClass", n=20, cfg = cfg, neuron_indices=rr_enhanced_neurons)

    # %% 可视化RR神经元分布 (抑制性)
    _rr_distribution_plot(neuron_pos_filtered, inhibitory_neuron_pos_rr, plot_dir, "Inhibitory_ByClass")
    # %% 可视化RR神经元响应 (抑制性)
    _plot_rr_responses(inhibitory_segments, labels, plot_dir, "Inhibitory_ByClass", n=20, cfg = cfg, neuron_indices=rr_inhibitory_neurons)
