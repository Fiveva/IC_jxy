...
from four_class import * # 重新定义配置（如果四分类代码未运行时需要）
# 确保 cfg 变量在整个脚本中是可用的。
...
def load_rr_neuron_indices_from_csv(file_path):
    """
    从 CSV 文件中读取 RR 神经元索引，只读取第一列（兴奋性神经元序号）。
    """
    ...
    try:
        # 尝试使用列名读取
        try:
            df = pd.read_csv(file_path, usecols=['兴奋性神经元序号'])
            column_name = '兴奋性神经元序号'
        except ValueError:
            # 如果没有列名，则读取第一列（索引 0）
            df = pd.read_csv(file_path)
            column_name = df.columns[0]
            print(f"警告: CSV文件缺少'兴奋性神经元序号'列，将使用第一列 '{column_name}'。")

        # 读取数据，去除 NaN/None 值，然后转换为整数列表
        indices = df[column_name].dropna().astype(int).tolist()
...
if __name__ == "__main__":
    ...
    csv_filename = 'rr_neuron_indices_summary.csv'
